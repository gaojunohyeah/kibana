/** @scratch /panels/5
 * include::panels/innergram.asciidoc[]
 */

/** @scratch /panels/innergram/0
 * == Innergram
 * Status: *Stable*
 *
 * The innergram panel allow for the display of time charts. It includes several modes and tranformations
...
define([
  ...
  function (angular, app, $, _, kbn, moment, timeSeries) {
    ...
    var module = angular.module('kibana.panels.innergram', []);
    app.useModule(module);

    module.controller('innergram', function ($scope, querySrv, dashboard, filterSrv) {
      $scope.panelMeta = {
        modals: [
          {
            description: "Inspect",
        ...
        editorTabs: [
          {
            title: 'Style',
            src: 'app/panels/innergram/styleEditor.html'
          },
          {
            title: 'Queries',
            src: 'app/panels/innergram/queriesEditor.html'
          },
        ],
        status: "Stable",
      ...
      $scope.get_data = function (data, segment, query_id) {
        ...
        var query_field_value_array = $scope.panel.query_field_values.split(' ');
        var j = 0;

        // Build the query
        _.each(query_field_value_array, function (value) {
          boolQuery = $scope.ejs.BoolQuery();
          _.each(queries, function (q) {
            var queryObject = JSON.parse(JSON.stringify(q));
            queryObject.query += ' AND ' + query_field_name + ':"' + value + '"';
            boolQuery = boolQuery.should(querySrv.toEjsObj(queryObject));
          });
          ...
          var facet = $scope.ejs.DateHistogramFacet(j);
...
//                        })
//                        _.each(queries, function (q) {
              var query_results = results.facets[i];
              ...
              $scope.legend[i] = {query: query_field_name + ":" + query_field_value_array[i],
                hits: hits,
                alias: JSON.parse(JSON.stringify(queries[0].alias)),
                color: querySrv.colors[i]};

              data[i] = {
                query: query_field_name + ":" + query_field_value_array[i],
                alias: JSON.parse(JSON.stringify(queries[0].alias)),
                color: querySrv.colors[i],
            ...
            // Tell the innergram directive to render.
    ...
    module.directive('innergramChart', function (dashboard, filterSrv) {
      return {
        restrict: 'A',
        template: '<div></div>',
        link: function (scope, elem) {
          ...
          elem.bind("plothover", function (event, pos, item) {
            ...
            if (item) {
              if (item.series.alias || scope.panel.tooltip.query_as_alias) {
                group = '<small style="font-size:0.9em;">' +
                  '<i class="icon-circle" style="color:' + item.series.color + ';"></i>' + ' ' +
                  (item.series.alias || item.series.query) +
                  '</small><br>';
              } else {
